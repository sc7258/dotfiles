# Generated by Powerlevel10k configuration wizard on 2026-04-03 at 11:12 KST.
...
function prompt_my_kube() {
  # 현재 접속된 K8s Context 이름 가져오기
  local ctx=$(kubectl config current-context 2>/dev/null)
  [[ -z "$ctx" ]] && return

  # $ctx 값에 따라 표시 방식과 색상 결정
  if [[ "$ctx" == "k8s-local" ]]; then
  # k8s-local일 때: telepresence 프로세스가 살아있을 때만 보라색(134)으로 띄움
  if pgrep -f "telepresence" > /dev/null; then
    p10k segment -f 134 -t "⛵ $ctx"
  fi
  elif [[ "$ctx" == "k8s-prod" ]]; then
    p10k segment -f 1 -t "🚨 $ctx"
  elif [[ "$ctx" == "k8s-qa" ]]; then
    p10k segment -f 208 -t "🧪 $ctx"
  elif [[ "$ctx" == "k8s-dev" ]]; then
    p10k segment -f 3 -t "🛠️ $ctx"
  else
    p10k segment -f 2 -t "☸ $ctx"
  fi
}
